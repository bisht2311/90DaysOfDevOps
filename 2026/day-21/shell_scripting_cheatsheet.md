...
- **Example**:
    ```bash
    echo $NAME # Prints Harshit
    ```
- **Quoting**:
 - **Double quotes " "**: Variable is replaced with its value.
    ```bash
    echo "Hello $NAME"   # Prints Hello Harshit
    ```
...
- $?: Last command status
 ...
 ```bash
    ...
    ls /Harshit                              # This directory does not exist
    ...
    ./script.sh Harshit                      # $1 = Harshit, $# = 1, $@ = Harshit
...
- **Syntax**:
    ...
    ```
- **Example**: Prints user IDs for root,ubuntu,and Harshit
    ```bash
    for USER in root ubuntu Harshit; do
        id "$USER"
    done
    ```
